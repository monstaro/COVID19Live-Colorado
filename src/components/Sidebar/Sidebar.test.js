...
describe("Sidebar", () => {
  it("should render each sidebar link", () => {
    ...
    expect(
      getByTestId("bookmarked-counties inactive-link")
    ).toBeInTheDocument();
  });
  it("should have an active class", async () => {
    ...
    const aboutActive = await waitForElement(() =>
      getByTestId("about active-link")
    );
    expect(aboutActive).toBeInTheDocument();
    fireEvent.click(getByTestId("staying-protected inactive-link"));
    const protectedActive = await waitForElement(() =>
      getByTestId("staying-protected active-link")
    );
    expect(protectedActive).toBeInTheDocument();
    fireEvent.click(getByTestId("local-health inactive-link"));
    const healthActive = await waitForElement(() =>
      getByTestId("local-health active-link")
    );
    expect(healthActive).toBeInTheDocument();
    fireEvent.click(getByTestId("county-stats inactive-link"));
    const statsActive = await waitForElement(() =>
      getByTestId("county-stats active-link")
    );
    expect(statsActive).toBeInTheDocument();
    fireEvent.click(getByTestId("bookmarked-counties inactive-link"));
    const bookmarksActive = await waitForElement(() =>
      getByTestId("bookmarked-counties active-link")
    );
